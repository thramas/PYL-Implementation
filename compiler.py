...
__author__ = "The Swag Programming Language Team"
__version__ = "0.1.0"
app_name = "Swag Programming Language"
project_url = "http://iiita.ac.in"

operators = ('+', '-', '*', '/', '=', '<>', '>', '<',
             '>=', '<=', 'and', 'or', 'mod', 'not')
enum = ('ADD', 'SUB', 'MUL', 'DIV', 'EQ',
        'NE', 'GT', 'LT', 'GE', 'LE', 'AND', 'OR', 'MOD', 'NOT')
...
def do_func(node):
    ...
    # (unless there's an user provided return!)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.gc:
            # used if we have the garbage colelction module installed
            cmd.append("-lgc")
        else:
            # set by default to ignore the garbage collector
        ...
        try:
            ...
            if p.wait() != 0:
                ...
                if any(e in errors for e in (b"-lgc",  b"gc.h:")):
                    print("*** gc not found, use --gc to enable the garbage collector, download the garbage colelctor from internet")
